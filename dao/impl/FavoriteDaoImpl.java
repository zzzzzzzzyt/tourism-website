...
public class FavoriteDaoImpl implements FavoriteDao {
    ...
    public List selectMyFavoriteByUid(int uid) {
        ...
        List<Myfavorite> myfavorites = template.query(sql, new BeanPropertyRowMapper<Myfavorite>(Myfavorite.class),uid);
        List<Route> list = new ArrayList<>();
        for (Myfavorite myfavorite : myfavorites) {
            int rid = myfavorite.getRid();
            Route route = new Route();
            String newsql = "select * from tab_route where rid = ? ";
            List<Route> routes = template.query(newsql, new BeanPropertyRowMapper<Route>(Route.class), rid);
